...
class ReferenceAccessibleScope internal constructor(private val uuid: UUID) {
    ...
    operator fun <Value> VarianceReference<Value>.invoke(value: Value) = set(uuid) { value }

    operator fun <Value> Reference<Value>.getValue(thisRef: Any?, property: KProperty<*>): Value = get(uuid)
    operator fun <Value> VarianceReference<Value>.setValue(thisRef: Any?, property: KProperty<*>, value: Value) = set(uuid) { value }
}